...
describe('EventEmitter', () => {
  ...
  describe('EventEmitter#asyncEmit', () => {

    it('should return false when there are not events to emit', () => {
      const e = new EventEmitter();

      assume(e.asyncEmit('foo')).equals(false);
      assume(e.asyncEmit('bar')).equals(false);
    });
    ...
    it('can emit the function with multiple arguments', () => {
      for (let i = 0; i < 100; i++) {
        (j => {
          ...
          e.once('args', () => assume(arguments.length).lte(args.length));

          e.asyncEmit.apply(e, ['args'].concat(args));
        })(i);
      }
    });
    ...
    it('should return true when there are events to emit', function (done) {
      ...
      assume(e.asyncEmit('foob')).equals(false);
      assume(e.asyncEmit('foo')).equals(true);
    });
    ...
    it('receives the emitted events', done => {
      ...
      e.asyncEmit('data', 'foo', e, new Date());
    });

    it('emits to all event listeners', (done) => {
      const e = new EventEmitter()
        , pattern = [];
